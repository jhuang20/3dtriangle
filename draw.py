...
def draw_polygons( polygons, screen, color ):
    if len(polygons)<2:
        return
        #print('need 3 points to draw')
    point=0
    v=[0,0,1]
    while point < len(polygons)-1:
        p=calculate_normal(polygons,point)
        p=normalize(p)
        #backface culling
        a=math.acos(dot_product(p,v))
        if a<math.pi/2:
        ...
        for longt in range(longt_start, longt_stop+1):
            ...
            if index+1>=len(points):
                ...
                add_polygon(polygons, points[index][0],
                     ...
                     points[index+1][1],
                     points[index+1][2],
                     points[(index+step)%step][0],
                     points[(index+step)%step][1],
                     points[(index+step)%step][2] )
            else:
                add_polygon(polygons, points[index][0],
                     points[index][1],
                     points[index][2],
